...
TMD5Stream = class(TStream)
  ...
  protected
    ...
    function GetDigestString: AnsiString; virtual;
  ...
  public
    ...
    property DigestString: AnsiString read GetDigestString;
  end;
...
function TMD5Stream.GetDigestString: AnsiString;
var
  D: TMD5Digest;
...
//  Move(D, Result[1], Length(Result))
  ...
  D := GetDigest;
  Move(D, tmp[1], Length(tmp));
  result := '';
  for i:=1 to 16 do
  begin
    result := result + AnsiString(IntToHex(Ord(tmp[i]), 2));
  end;
end;
